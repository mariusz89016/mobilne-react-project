...
import sri.mobile.template.components.RegistrationComponent.State
...
@ScalaJSDefined
class RegistrationComponent extends ReactComponent[RegistrationComponent.Props, RegistrationComponent.State] {
  initialState(State("", "", ""))
...
object RegistrationComponent {
  ...
  def apply(props: Props): ReactElementU[Props, State] = makeElement[RegistrationComponent](props)
}
